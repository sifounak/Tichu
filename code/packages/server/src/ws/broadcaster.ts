...
export class Broadcaster {
  ...
  broadcastGameState(roomCode: string, context: GameMachineContext, machineState: string, vacatedSeats: readonly Seat[] = [], choosingSeats: readonly Seat[] = [], disconnectVoteStatus?: { votes: Record<string, 'wait' | 'kick' | null>; disconnectedSeats: Seat[]; timeoutMs: number } | null, activeVote?: { voteId: string; voteType: 'kick' | 'restartGame' | 'restartRound'; initiatorSeat: Seat; targetSeat?: Seat; votes: Record<string, boolean | null>; timeoutMs: number } | null, timerInfo?: { startTime: number | null; durationMs: number | null }, endOfTrickBombWindowEndTime?: number | null, waitingForReconnect?: Seat | null, disconnectedSeats?: Seat[], kickDialog?: { targetSeat: Seat } | null): number {
    ...
    for (const { ws, info } of clients) {
      if (info.seat) {
        const view = projectGameState(context, machineState, info.seat, vacatedSeats, choosingSeats, disconnectVoteStatus, activeVote, timerInfo, endOfTrickBombWindowEndTime, waitingForReconnect, disconnectedSeats, kickDialog);
        ...
        if (!spectatorView) {
          spectatorView = projectSpectatorView(context, machineState, vacatedSeats, disconnectVoteStatus, activeVote, timerInfo, endOfTrickBombWindowEndTime, waitingForReconnect, disconnectedSeats, kickDialog);
        }
