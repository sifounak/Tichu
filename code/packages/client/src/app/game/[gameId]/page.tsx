...
function formatCompactTichuCall(call: string): string {
  if (call === 'blindGrandTichu') return 'Blind';
  if (call === 'grandTichu') return 'Grand';
  return 'Tichu';
}
    ...
    <>
      ...
      <div data-debug-area="Control Panel" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <SpectatorPresence spectatorCount={spectatorCount} spectatorNames={spectatorNames} players={roomPlayers} />
        </div>
      ...
      {isMobileLayout && (
        <div data-debug-area="Mobile Chrome" style={{
          ...
          <div style={{ pointerEvents: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 'var(--space-2)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
              ...
              <SpectatorPresence spectatorCount={spectatorCount} spectatorNames={spectatorNames} players={roomPlayers} compact />
            </div>
      ...
      {phase !== GamePhase.WaitingForPlayers && !isSpectator && (
        <div data-debug-area="Bottom Panel" style={{ position: 'fixed', bottom: 'calc(34px * var(--scale))', left: 0, right: 0, zIndex: 26, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'calc(28px * var(--scale))', pointerEvents: 'none', '--card-width': layoutTier === 'mobile' ? '14vw' : 'var(--card-width-lg)', '--card-height': layoutTier === 'mobile' ? '20vw' : 'var(--card-height-lg)' } as React.CSSProperties}>
              ...
              <div style={{ width: 'var(--mobile-btn-narrow-width)', flexShrink: 0, pointerEvents: 'auto' }}>
                {(phase === 'playing' || phase === 'cardPassing' || ((phase === 'grandTichuDecision' || phase === 'blindGrandTichuDecision') && mySeat && gameStore.grandTichuDecided.includes(mySeat))) && !gameStore.gameHalted && gameStore.myTichuCall === 'none' && !gameStore.hasPlayedCards && view.finishOrder.length === 0 && (
                  <button
                    onClick={handleTichu}
                    style={{
                      width: 'var(--mobile-btn-narrow-width)',
          ...
          {isMobileLayout && !isPreGame && !showReceivedCards && gameStore.myTichuCall !== 'none' && (() => {
            ...
            const label = formatCompactTichuCall(gameStore.myTichuCall);
          ...
          {isMobileLayout && (isPreGame || showReceivedCards) && gameStore.myTichuCall !== 'none' && (
            ...
            <div style={{
              ...
              {formatCompactTichuCall(gameStore.myTichuCall)}
            </div>
              );
            })()
          )}
